...
const CHAR_INDENT = "\t";
var logIndent = 0;
// console.log({this:this,process:process.execPath.split("/home/fishy/")});
...
var threeSums = function (aNumsArray, target) {
    ...
    if (aNumsArray.length < SET_SIZE) {
        writeToVerboseBullet(`EARLY EMPTY EXIT: a nums of size ${aNumsArray.length} is not enough to form a triplet`);
        return emptyResult;
    }
    ...
    if (target < nums[0]) {
        writeToVerboseBullet(`EARLY EMPTY EXIT: Target (${target}) is lower than the smallest value in nums - ${nums[0]}`);
        return emptyResult;
    }
    ...
    while (idxI < nums.length) {
        ...
        if (numsSliced.length < SET_SIZE - 1) {
            writeToVerboseBullet(`WHILE BREAK: a numsSliced of size ${numsSliced.length} is not enough to form a duo`);
            break;
        }
...
var twoSums = (aNumsArray, target) => {
    ...
    while (idxJ < idxK) {
        ...
        //     writeToVerboseBullet(`WHILE BREAK: Target (${target}) is now unreachable. Sum(j:${valJ},k:${valK})=${sumJK} for [j,k]=[${idxJ},${idxK}] is off target by ${valJOffByTarget}`);
        //     break;
        // }
        if (sumOffByTarget === 0) {
    ...
    for (const entry of iterator1) {
        ...
        writeToVerboseBullet(`Validated Duo [${valJ},${valK}] in [${nums.length < 10 ? nums : "nums"}] for [j,k]=[${idxJ},${idxK}]`);
